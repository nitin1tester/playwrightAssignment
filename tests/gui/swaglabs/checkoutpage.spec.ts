...
for (const user of usersdata) {
    test(`Positive: Complete checkout flow ${user.firstName} `, async ({ inventoryPage }) => {
        ...
        const userCartPage: UserCartPage = await inventoryPage.clickOnShoppingCartBtn();
        const checkoutUserInfoPage: CheckoutUserInfoPage = await userCartPage.clickOnUserCartCheckoutBtn();
        ...
        const checkoutOverviewPage: CheckoutOverviewPage = await checkoutUserInfoPage.clickContinueBtn();
        const checkoutCompletePage: CheckoutCompletePage = await checkoutOverviewPage.clickFinishBtn();
        const isOrderConfirmed = await checkoutCompletePage.isOrderConfirmationMsgVisible();
